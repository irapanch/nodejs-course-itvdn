...
function processUpdateCommentRes(updateComment) {
  console.log(updateComment);
}
function processCommentRes(comments) {
  console.log(comments[0]);
  updateCommentBody(
    comments[0].id,
    "New text for comment",
    processUpdateCommentRes
  );
}
function processPostsRes(posts) {
  console.log(posts[0]);
  getPostComments(posts[0].id, processCommentRes);
}
getUserPosts(USER_ID, processPostsRes);
